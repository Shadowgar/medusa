/*
	...
	Message-Digest Algorithm (Note: more comments at the end of this file)

	This class can be used to generate a checksum from a byte buffer. This class is typically
	used to hash a private key and public key together. 

	1. Client connects to server, server sends a random public key accross the internet.
	2. Client hashes the public key with the password (private key) the user has entered, this checksum is
	sent back to the server.
	3. The server compares the received checksum against it's own calculated checksum, if the values
...
public:
	...
	MD5( const byte * Input, int nInputLen );

	// Mutators
	void			initialize();
	void			digest( const byte * Input, unsigned int nInputLen );
	CharString		checksum();	
...
private:
	...
	void			DWordToByte(byte* Output, const dword* Input, unsigned int nLength);
	void			ByteToDWord(dword* Output, const byte * Input, unsigned int nLength);
